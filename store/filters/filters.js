...
export const actions = {
    ...
    async getFilters({commit, state}, payload) {
        ...
        let removeEmptyObjects = function (obj) {
            return _(obj)
        ...
        let StringToArray = (input) => Array.isArray(input) ? [...input] : (input ? [input] : []);
        let NumberToArray = (input) => Array.isArray(input) ? [...input.map(item => Number(item))] : (input ? [Number(input)] : []);
        ...
        if (payload !== 'changeUrl') {
            ...
            variables = {
                site_id: $nuxt.context.$config.site_id,
                url: url.path,
                mark_slug_array: StringToArray(url.query.mark_slug_array),
                folder_slug_array: StringToArray(url.query.folder_slug_array),
                generation_slug_array: StringToArray(url.query.generation_slug_array),
                engine_type_id_array: NumberToArray(url.query.engine_type_id_array),
                gearbox_id_array: NumberToArray(url.query.gearbox_id_array),
                drive_type_id_array: NumberToArray(url.query.drive_type_id_array),
                body_type_id_array: NumberToArray(url.query.body_type_id_array),
            }
        }
        ...
        try{
            let response = await client.query(
                {
                    query: offerFilters,
                    variables: removeEmptyObjects(variables),
