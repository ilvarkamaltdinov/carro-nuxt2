  ...
  <section class="page-main__application application grid">
    ...
    <div class="grid__col-12 grid grid--application">
      <div class="application__form grid__col-4">
        <form-credit calculator :bank-rate="bank.rate" />
      </div>
      <div v-if="currentCar && !$device.isMobile" class="application__catalog catalog grid__col-8">
        <component :choose="true" :is="component" :has-fancy="true" :key="currentCar.id" :is-form="true"
          :has-buttons="false" :offer="currentCar" />
      </div>

      <button v-else class="application__choose-car grid__col-8" @click="chooseCar">
        <svg-icon class="application__choose-car-icon" name="icon-form" />
